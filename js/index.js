...
var filteredSearchDrugs = new Array()
/**
...
function filterSearch() {
    ...
    for (i = 0; i < li.length; i++) {
        ...
        if (txtValue.toUpperCase().indexOf(filter) > -1) {
            if (li[i].style.display !== "none" && filteredSearchDrugs.indexOf(li[i].innerHTML) === -1) {  //fix this logic
                li[i].style.display = "";
                filteredSearchDrugs.splice(filteredSearchDrugs.indexOf(li[i].innerHTML), 1)
            }
        } else {
          li[i].style.display = "none";
          filteredSearchDrugs.push(li[i].innerHTML)
        }
    }
}
...
function updateSearch (drugs) {
    ...
    for (i = 0; i < li.length; i++) {
        a = li[i].getElementsByTagName("a")[0];
        txtValue = a.textContent || a.innerText;
        if (drugs.indexOf(txtValue.toUpperCase()) > -1) {
            li[i].style.display = "";
        } else {
            li[i].style.display = "none";
        }
